...
object ReplicatedMetrics {
  ...
  def apply[A <: Nothing]: Behavior[A] = apply[A](1.second, 1.minute)

  def apply[A <: Nothing](measureInterval: FiniteDuration, cleanupInterval: FiniteDuration): Behavior[A] =
    Behaviors.setup[Command] { context =>
      DistributedData.withReplicatorMessageAdapter[Command, LWWMap[String, Long]] { replicator =>
        Behaviors.withTimers[Command] { timers =>
          ...
          def postStop(): Unit = {
            timers.cancel(Tick)
            cluster.subscriptions ! Unsubscribe(memberUpRef)
            cluster.subscriptions ! Unsubscribe(memberDownRef)
          }
